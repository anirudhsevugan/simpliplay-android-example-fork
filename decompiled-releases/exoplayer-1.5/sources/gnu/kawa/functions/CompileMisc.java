...
public class CompileMisc implements Inlineable {
    ...
    public static void compileCallCC(ApplyExp exp, Compilation comp, Target target, Procedure proc2) {
        ...
        if (!param.isSimple() || param.getCanRead() || param.getCanWrite()) {
            Scope sc = code2.pushScope();
            ClassType classType = typeContinuation;
            Variable contVar = sc.addVariable(code2, classType, (String) null);
            ...
            new ApplyExp((Expression) lambda, new ReferenceExp(contDecl)).compile(compilation, target2);
            if (code2.reachableHere()) {
            ...
            if (target2 instanceof ConsumerTarget) {
                ...
                target2.compileFromStack(compilation, Type.objectType);
            }
            code2.emitCatchEnd();
        ...
        new ApplyExp((Expression) lambda, QuoteExp.nullExp).compile(compilation, target2);
